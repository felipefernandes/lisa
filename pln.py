    ...
    if coluna_titulo:
        df["Embeddings"] = df.apply(
            lambda row: genai.embed_content(
                model=model, content=row[coluna_conteudo], title=row[coluna_titulo], task_type="RETRIEVAL_DOCUMENT")["embedding"],
            axis=1
        )
    else:
        df["Embeddings"] = df.apply(
            lambda row: genai.embed_content(
                model=model, content=row[coluna_conteudo], task_type="RETRIEVAL_DOCUMENT")["embedding"],
            axis=1
        )
    return df
